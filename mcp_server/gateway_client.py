"""GatewayClient: wrapper over gRPC stub to Gateway."""
...
class GatewayClient:
    ...
    async def _get_stub(self) -> gateway_pb2_grpc.GatewayServiceStub:
        if self._stub is None:
            logger.debug("MCP Gateway channel created addr=%s", self._addr)
    ...
    async def get_dress_advice(
        ...
        return await s.GetDressAdvice(
            gateway_pb2.GetDressAdviceRequest(
                user_id=user_id, city_name=city_name, date=date, time=time, locale=locale
            )
        )
